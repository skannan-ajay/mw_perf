...
BENCHMARK_DEFINE_F(ContainerFixture, bmBinarySearch)(benchmark::State& aState) {
  ContainerFixture::Setup(aState);
  auto data = ContainerFixture::getData();
  BinarySearch<ContainerType> searcher(data);
...
BENCHMARK_REGISTER_F(ContainerFixture, bmBinarySearch)
    ->RangeMultiplier(10)
    ->Range(1e3, 1e6)
